...
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QTextEdit, QGroupBox, QRadioButton, QComboBox, QSlider, QProgressBar
)
from utils.video_encoder import VideoEncoder

class VideoTranscodeTab(QWidget):
    def __init__(self):
        super().__init__()
        self.init_ui()
        self.process = None
    ...
    def init_ui(self):
        ...
        self.supported_formats = ["mp4", "mkv", "avi", "mov", "flv", "wmv", "webm", "mpeg", "mpg"]
        ...
        # 编码选项区域
        encoding_options_group = QGroupBox("编码选项")
        ...
        encoding_options_group.setLayout(encoding_options_layout)

        # 操作按钮
        button_layout = QHBoxLayout()
        self.start_button = QPushButton("开始转码")
        ...
        layout.addWidget(encoding_options_group)
    ...
    def select_video_file(self):
        ...
        if file_path:
            ...
            if not self.output_name_edit.text().strip():
                ...
                self.output_name_edit.setText(f"{base_name}_transcoded")
    ...
    def on_processing_type_changed(self):
        if self.cpu_radio.isChecked():
            self.gpu_brand_layout_widget.setVisible(False)
            self.threads_layout.itemAt(1).widget().setVisible(True)
            # 更新编码器为CPU编码器
            ...
            self.threads_layout.itemAt(1).widget().setVisible(False)
    ...
    def start_transcoding(self):
        ...
        if self.output_name_edit.text().strip():
            ...
            base_name = os.path.splitext(os.path.basename(video_path))[0] + "_transcoded"
